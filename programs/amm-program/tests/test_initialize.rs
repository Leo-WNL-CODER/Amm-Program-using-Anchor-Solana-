...
fn test_initialize() {
    ...
    let counter = Pubkey::find_program_address(
        &[amm_program::constants::COUNTER_SEED],
        &program_id,
    )
    .0;
    ...
    let instruction = Instruction::new_with_bytes(
        program_id,
        &amm_program::instruction::Initialize {}.data(),
    ...
    let counter_state = amm_program::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &amm_program::instruction::Increment {}.data(),
        amm_program::accounts::Increment {
            counter,
